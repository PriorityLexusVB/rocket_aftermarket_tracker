...
-- Drop potentially problematic policies that may cause recursion
DO $$
BEGIN
  -- Drop old policies that may reference auth_user_org()
  ...
  -- Keep: "own profile read", "own profile read by email via jwt", "users_manage_own_user_profiles"
  RAISE NOTICE 'Dropped old user_profiles policies that may cause recursion';
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Some policies already dropped or do not exist';
END $$;

-- Policy: user_profiles_select_active - All authenticated can see active profiles
...
BEGIN
  ...
  CREATE POLICY "user_profiles_select_active" ON public.user_profiles
    FOR SELECT TO authenticated
    USING (COALESCE(is_active, true));
  RAISE NOTICE 'Created policy: user_profiles_select_active';
END $$;

-- Policy: user_profiles_self_read - Users can always read their own profile
DO $$
BEGIN
  ...
  RAISE NOTICE 'Created policy: user_profiles_self_read';
END $$;

-- Policy: user_profiles_self_update - Users can update their own profile
DO $$
BEGIN
  ...
  RAISE NOTICE 'Created policy: user_profiles_self_update';
END $$;

-- Policy: user_profiles_self_insert - Users can insert their own profile (for initial setup)
DO $$
BEGIN
  ...
  RAISE NOTICE 'Created policy: user_profiles_self_insert';
END $$;

-- Keep existing "own profile read" and related policies that use auth.uid() directly
-- These are safe and don't cause recursion
...
-- Ensure org members can read products (with fallback for NULL org)
DO $$
BEGIN
  ...
  RAISE NOTICE 'Created policy: app_org_staff_can_read_products';
END $$;
...
BEGIN
  ...
  RAISE NOTICE 'Created policy: app_org_staff_can_read_vendors';
END $$;
...
BEGIN
  ...
  RAISE NOTICE 'Created/updated policy: org can select loaner_assignments via jobs';
END $$;
