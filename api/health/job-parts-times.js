...
export default async function handler(req, res) {
  ...
  try {
    ...
    const key = envKey();
    if (!supabaseUrl || !key) return sendJson(res, 500, { ok: false, error: "Missing SUPABASE_URL/VITE_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY/VITE_SUPABASE_ANON_KEY", ms: Date.now() - t0 });
    const columns = {
