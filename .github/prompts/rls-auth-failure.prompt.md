...
- Network: scan the last ~80 requests.
...
- If the problem might be env/config rather than RLS, instruct to run `scripts/verify-schema-cache.sh` and/or `node scripts/verify-capabilities.js` and/or hit `/api/health/*` endpoints to confirm app/Supabase connectivity.
