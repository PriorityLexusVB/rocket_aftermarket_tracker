...
test.describe('Deals List Refresh After Edit', () => {
  ...
  test('should show updated vehicle description, stock, and loaner badge in deals list', async ({
    page,
  }) => {
    ...
    const vehicleCell = page
      .locator(`[data-testid="deal-row-${cleanDealId}"] [data-testid*="vehicle"]`)
      .first()
    const initialVehicleText = await vehicleCell.textContent().catch(() => '')

    const loanerBadgeExists = await page
      .locator(`[data-testid="deal-row-${cleanDealId}"] [data-testid*="loaner-badge"]`)
      .isVisible()
      .catch(() => false)
    ...
    await expect(page.locator(`[data-testid="deal-row-${cleanDealId}"]`)).toBeVisible({
      timeout: 10_000,
    })
    ...
    if (newStockNumber) {
      const stockCell = page.locator(
        `[data-testid="deal-row-${cleanDealId}"] [data-testid*="stock"], [data-testid="deal-row-${cleanDealId}"] :has-text("${newStockNumber}")`
      )
    ...
    if (newLoanerState !== null) {
      const loanerBadgeAfter = await page
        .locator(
          `[data-testid="deal-row-${cleanDealId}"] [data-testid*="loaner-badge"], [data-testid="deal-row-${cleanDealId}"] [title*="loaner"], [data-testid="deal-row-${cleanDealId}"] :has-text("🚗")`
        )
        .isVisible()
        .catch(() => false)
  ...
  test('should update promised date/window in deals list after edit', async ({ page }) => {
    ...
    const dateField = page.locator(
      `[data-testid="deal-row-${cleanDealId}"] [data-testid*="date"], [data-testid="deal-row-${cleanDealId}"] time, [data-testid="deal-row-${cleanDealId}"] :text-matches("\\d{4}-\\d{2}-\\d{2}|\\d{1,2}/\\d{1,2}/\\d{4}")`
    )
