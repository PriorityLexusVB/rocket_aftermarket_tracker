...
describe('Step 14: Edit flow - Test add/update/delete line items and verify DB persistence', () => {
      ...
      from: jest?.fn((tableName) => ({
        ...
        select: jest?.fn(() => {
          const chain = {
            ...
            order: jest?.fn(() =>
              Promise.resolve({
                data: mockDataUpdated
                  ? getUpdatedMockData(tableName)
                  : getExistingMockData(tableName),
                error: null,
              })
            ),
            then: (resolve) =>
  ...
  test('✓ Verify DB results: edited parts after update/add/delete', async () => {
    ...
    if (updatedPart) {
      ...
      console.log(
        '   - Updated part: new product/price, quantity_used=1, promised date persisted ✓'
      )
    }
  ...
  test('✓ Verify DB results: roll-up amount updated in transactions', async () => {
    ...
    const transactionQuery = await mockSupabaseClient
      ?.from('transactions')
      ?.select('total_amount')
      ?.eq('job_id', existingJobId)
  ...
  test('✓ Verify quantity handling: UI vs DB persistence', async () => {
    ...
    console.log(
      `   - Total price calculation: $${unitPrice} × ${finalDbQuantity} = $${expectedTotalPrice} ✓`
    )
...
FROM job_parts WHERE job_id = '${existingJobId}' ORDER BY id;`)
    ...
    console.log(
      "   1. ✓ Changed existing line item's product/price/quantity (qty saves as 1 in DB)"
    )
