...
describe('Step 20: RLS & Multi-User Concurrency', () => {
  ...
  beforeAll(async () => {
    ...
    const { data: users } = await supabase?.from('user_profiles')?.select('*')?.in('role', ['staff', 'manager', 'admin'])?.limit(3);
    ...
    const { data: jobPart, error: partError } = await supabase?.from('job_parts')?.insert({
        job_id: testJobId,
  ...
  describe('Staff User A (Job Owner)', () => {
    test('can view and edit own assigned job', async () => {
      ...
      const { data: viewJob, error: viewError } = await supabase?.from('jobs')?.select('*')?.eq('id', testJobId)?.single();
      ...
      const { data: updatedJob } = await supabase?.from('jobs')?.select('description')?.eq('id', testJobId)?.single();
    ...
    test('can view job parts but cannot manage them (manager only)', async () => {
      ...
      const { data: jobParts, error: viewError } = await supabase?.from('job_parts')?.select('*')?.eq('job_id', testJobId);
      ...
      const { error: updateError } = await supabase?.from('job_parts')?.update({ unit_price: 150 })?.eq('id', testJobPartsId);
  ...
  describe('Staff User B (Different Staff Member)', () => {
    test('can view job but cannot edit it (not assigned)', async () => {
      ...
      const { data: unchangedJob } = await supabase?.from('jobs')?.select('description')?.eq('id', testJobId)?.single();
    ...
    test('cannot manage job parts (manager only policy)', async () => {
      ...
      const { error: updateError } = await supabase?.from('job_parts')?.update({ unit_price: 200 })?.eq('id', testJobPartsId);
  ...
  describe('Manager User', () => {
    ...
    test('can manage job parts (full access)', async () => {
      ...
      const { error: updateError } = await supabase?.from('job_parts')?.update({ unit_price: 250 })?.eq('id', testJobPartsId);

      expect(updateError)?.toBeNull();

      // Verify the update was successful
      const { data: updatedPart } = await supabase?.from('job_parts')?.select('unit_price')?.eq('id', testJobPartsId)?.single();
