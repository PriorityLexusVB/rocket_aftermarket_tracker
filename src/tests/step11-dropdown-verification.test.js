...
describe('Step 11: Dropdowns Verification', () => {
  ...
  describe('Dropdown Data Source Verification', () => {
    test('should verify salesperson dropdown sources and filters', async () => {
      ...
      // Run SQL probe
      const { data: probeResult, error: probeError } = await supabase?.rpc('execute_sql', { sql: sqlProbes?.salesConsultants })?.single();

      if (probeError) {
        // Fallback to direct query if SQL probe function doesn't exist
        const { data: salesConsultants, error: queryError } = await supabase?.from('user_profiles')?.select('full_name')?.eq('role', 'staff')?.eq('department', 'Sales Consultants')?.eq('is_active', true)?.order('full_name');
    ...
    test('should verify delivery coordinator dropdown sources and filters', async () => {
      ...
      const { data: coordinators, error } = await supabase?.from('user_profiles')?.select('full_name, role, department')?.in('role', ['admin', 'manager'])?.eq('department', 'Delivery Coordinator')?.eq('is_active', true)?.order('full_name');
    ...
    test('should verify vendor dropdown sources and filters', async () => {
      ...
      const { data: vendors, error } = await supabase?.from('vendors')?.select('name, is_active')?.eq('is_active', true)?.order('name');
    ...
    test('should verify product dropdown sources and filters', async () => {
      ...
      const { data: products, error } = await supabase?.from('products')?.select('name, is_active')?.eq('is_active', true)?.order('name');
  ...
  describe('Edit Persistence Verification', () => {
    test('should create deal with line items and verify edit persistence', async () => {
      ...
      const [vendorsResult, productsResult, salesResult, coordResult] = await Promise.all([
        ...
        supabase?.from('user_profiles')?.select('id, full_name')?.eq('role', 'staff')?.eq('department', 'Sales Consultants')?.eq('is_active', true)?.limit(1),
        supabase?.from('user_profiles')?.select('id, full_name')?.in('role', ['admin', 'manager'])?.eq('department', 'Delivery Coordinator')?.eq('is_active', true)?.limit(1)
      ]);
      ...
      const { data: editJob, error: editError } = await supabase?.from('jobs')?.select(`
          *,
          job_parts (
            *,
            products (name)
      ...
      if (coordinator) {
        ...
        console.log(`    ✅ Delivery Coordinator preserved: ${editJob?.delivery_coordinator?.full_name}`);
      }
      ...
      console.log(`    ✅ In-house item no-schedule reason preserved: ${inHouseItem?.no_schedule_reason}`);
      ...
      console.log('    ✅ All dropdown selections and line item configurations preserved in edit mode');
    });
  });
  ...
  describe('SQL Probe Summary', () => {
    test('should run all SQL probes and print summary', async () => {
      ...
      try {
        const { data: salesData } = await supabase?.from('user_profiles')?.select('full_name')?.eq('role', 'staff')?.eq('department', 'Sales Consultants')?.eq('is_active', true);
      ...
      try {
        const { data: coordData } = await supabase?.from('user_profiles')?.select('full_name')?.in('role', ['admin', 'manager'])?.eq('department', 'Delivery Coordinator')?.eq('is_active', true);
      ...
      try {
        const { data: vendorData } = await supabase?.from('vendors')?.select('name')?.eq('is_active', true);
      ...
      try {
        const { data: productData } = await supabase?.from('products')?.select('name')?.eq('is_active', true);
      ...
      console.log('\n  📊 SQL Probe Results:');
      console.log(`    Sales Consultants (role='staff', dept='Sales Consultants', active=true): ${results?.salesConsultants?.count} | Sample: ${results?.salesConsultants?.sample}`);
      console.log(`    Delivery Coordinators (role in admin/manager, dept='Delivery Coordinator', active=true): ${results?.deliveryCoordinators?.count} | Sample: ${results?.deliveryCoordinators?.sample}`);
      console.log(`    Active Vendors (is_active=true): ${results?.activeVendors?.count} | Sample: ${results?.activeVendors?.sample}`);
      console.log(`    Active Products (is_active=true): ${results?.activeProducts?.count} | Sample: ${results?.activeProducts?.sample}`);
