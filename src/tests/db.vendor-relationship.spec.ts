...
describe('Database Vendor Relationship - REST API Integration', () => {
  ...
  const IS_LOCALHOST = process.env.VITE_SUPABASE_URL?.includes('localhost') || 
  ...
  it('should have vendor_id column on job_parts table', async () => {
    if (!INTEGRATION_MODE) {
      console.log('   ⊘ Skipped (mock mode - set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY for integration test)')
  ...
  it('CRITICAL: should support nested vendor relationship query without error', async () => {
    ...
    if (error) {
      ...
      // Unexpected error
      throw new Error(`Unexpected error querying job_parts with vendor relationship: ${errorMessage}`)
    }
  ...
  it('should document the verification process for manual testing', () => {
    ...
    const verificationSteps = {
      step1: 'Check column exists: SELECT column_name FROM information_schema.columns WHERE table_name = \'job_parts\' AND column_name = \'vendor_id\'',
      step2: 'Check FK constraint: SELECT constraint_name FROM information_schema.table_constraints WHERE table_name = \'job_parts\' AND constraint_name = \'job_parts_vendor_id_fkey\'',
      step3: 'Reload cache: NOTIFY pgrst, \'reload schema\';',
      step4: 'Test query: curl "${SUPABASE_URL}/rest/v1/job_parts?select=id,vendor:vendors(id,name)&limit=1" -H "apikey: ${ANON_KEY}"',
...
describe('Database Vendor Relationship - Mock Mode Tests', () => {
  ...
  it('should define expected error message for missing relationship', () => {
    const expectedError = "Could not find a relationship between 'job_parts' and 'vendors' in the schema cache"
