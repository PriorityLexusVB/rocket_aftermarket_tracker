...
describe('Step 8: Create → Edit Round-trip Test', () => {
  ...
  beforeAll(async () => {
    // Create test user
    const { data: user, error: userError } = await supabase?.from('user_profiles')?.insert([{
    ...
    for (const product of productsToInsert) {
      const { data: productData, error: productError } = await supabase?.from('products')?.insert([product])?.select('id')?.single();
  ...
  test('Create deal with two line items (Off-Site with promised date, On-Site with no schedule reason)', async () => {
    ...
    const { data: transaction, error: txnError } = await supabase?.from('transactions')?.select('*')?.eq('job_id', createdDealId)?.single();
  ...
  test('Edit deal: add new item, change price, set promised date', async () => {
    ...
    const updatedOffSiteItem = updatedDeal?.job_parts?.find(item => 
      item?.product_id === testProductIds?.[0]
    );
    ...
    const unchangedOnSiteItem = updatedDeal?.job_parts?.find(item => 
      item?.product_id === testProductIds?.[1]
    );
    ...
    const newItem = updatedDeal?.job_parts?.find(item => 
      item?.product_id === testProductIds?.[2]
    );
    ...
    const { data: updatedTransaction, error: txnError } = await supabase?.from('transactions')?.select('*')?.eq('job_id', createdDealId)?.single();
  ...
  test('Verify final state integrity', async () => {
    ...
    const { data: allJobParts, error: partsError } = await supabase?.from('job_parts')?.select('*')?.eq('job_id', createdDealId)?.order('unit_price', { ascending: true });
    ...
    const { data: finalTransaction } = await supabase?.from('transactions')?.select('*')?.eq('job_id', createdDealId)?.single();
  ...
  test('Complete workflow summary', () => {
    ...
    console.log('✅ Created deal with 2 line items (1 Off-Site with promised date, 1 On-Site with no schedule reason)');
