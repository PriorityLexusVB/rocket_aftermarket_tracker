...
describe('Step 13: Persistence - Create deal with 2 items and verify correct DB rows written', () => {
  ...
  beforeEach(() => {
    ...
    testJobIds = ['test-job-id-1', 'test-job-id-2']
    ...
    mockSupabaseClient = {
      from: jest?.fn((tableName) => ({
        ...
        select: jest?.fn(() => ({
          // When used as a terminal call (await ...eq(...)), return data directly
          eq: jest?.fn(() => ({
            data: getSelectMockData(tableName),
            error: null,
            // Still allow chaining .order(...).limit(...) if present elsewhere
            order: jest?.fn(() =>
              Promise.resolve({
                data: getSelectMockData(tableName),
                error: null,
  ...
  test('✓ Verify database results: jobs table', async () => {
    ...
    const jobsQuery = await mockSupabaseClient
      ?.from('jobs')
      ?.select(
        `
  ...
  test('✓ Verify database results: job_parts table', async () => {
    ...
    const jobPartsQuery = await mockSupabaseClient
      ?.from('job_parts')
      ?.select(
        `
  ...
  test('✓ Verify database results: transactions table', async () => {
    ...
    const transactionsQuery = await mockSupabaseClient
      ?.from('transactions')
      ?.select(
        `
    ...
    console.log(
      `   📊 Found ${transactionResults?.length} transactions for jobs: ${testJobIds?.join(', ')}`
    )
  ...
  test('✓ Verify database results: vehicles owner info', async () => {
    ...
    // Mock vehicles query
    const vehiclesQuery = await mockSupabaseClient
      ?.from('vehicles')
