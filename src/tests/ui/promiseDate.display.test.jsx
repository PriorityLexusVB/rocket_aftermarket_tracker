...
describe('Promise Date Display - UI Safety', () => {
  ...
  describe('formatTimeWindow', () => {
    it('should display "Time TBD" when start is null', () => {
      expect(formatTimeWindow(null, '2025-01-15T10:00:00')).toBe('Time TBD')
    })

    it('should display "Time TBD" when end is null', () => {
      expect(formatTimeWindow('2025-01-15T09:00:00', null)).toBe('Time TBD')
    })

    it('should display "Time TBD" when both are null', () => {
      expect(formatTimeWindow(null, null)).toBe('Time TBD')
    })

    it('should display "Time TBD" when both are empty strings', () => {
      expect(formatTimeWindow('', '')).toBe('Time TBD')
    })

    it('should display "Time TBD" when start is invalid', () => {
      expect(formatTimeWindow('invalid', '2025-01-15T10:00:00')).toBe('Time TBD')
    })

    it('should display "Time TBD" when end is invalid', () => {
      expect(formatTimeWindow('2025-01-15T09:00:00', 'invalid')).toBe('Time TBD')
    })
  ...
  describe('UI Integration Patterns', () => {
    ...
    it('should safely display scheduled window in calendar/appointment views', () => {
      ...
      expect(displayValues[0]).toBe('Time TBD')
      expect(displayValues[1]).toBe('Time TBD')
      expect(displayValues[2]).not.toBe('Time TBD')
      expect(displayValues[3]).toBe('Time TBD')
    ...
    it('should handle mixed valid/invalid dates in a deal', () => {
      ...
      expect(jobWindow).toBe('Time TBD') // end time is missing
