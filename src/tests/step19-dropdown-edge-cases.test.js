...
describe('Step 19: Dropdown Edge Cases', () => {
  ...
  afterAll(async () => {
    ...
    if (testData?.deactivatedProduct && testData?.originalProductState !== null) {
      await supabase
        ?.from('products')
        ?.update({ is_active: testData?.originalProductState })
        ?.eq('id', testData?.deactivatedProduct?.id)
    }

    if (testData?.deactivatedVendor && testData?.originalVendorState !== null) {
      await supabase
        ?.from('vendors')
        ?.update({ is_active: testData?.originalVendorState })
        ?.eq('id', testData?.deactivatedVendor?.id)
    }
  ...
  test('should temporarily deactivate product and vendor used by existing deals', async () => {
    ...
    // Verify deactivation
    const { data: deactivatedProduct } = await supabase
      ?.from('products')
      ?.select('is_active, name')
      ?.eq('id', testData?.deactivatedProduct?.id)
      ?.single()

    const { data: deactivatedVendor } = await supabase
      ?.from('vendors')
      ?.select('is_active, name')
      ?.eq('id', testData?.deactivatedVendor?.id)
      ?.single()
  ...
  test('should execute SQL probes as specified in requirements', async () => {
    ...
    const { data: vendorStatus } = await supabase
      ?.from('vendors')
      ?.select('name, is_active')
      ?.eq('id', testData?.deactivatedVendor?.id)
      ?.single()
  ...
  test('should demonstrate complete Step 19 PASS criteria', async () => {
    ...
    const { data: existingJobPart } = await supabase
      ?.from('job_parts')
