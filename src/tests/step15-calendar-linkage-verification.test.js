...
describe('Step 15: Calendar Linkage Verification', () => {
  ...
  describe('Off-site Job Calendar Fields', () => {
    it('should have scheduled_start_time and scheduled_end_time populated', async () => {
      ...
      const { data: offSiteJobs, error } = await supabase?.from('jobs')?.select('id, title, service_type, scheduled_start_time, scheduled_end_time, calendar_event_id, location, color_code')?.eq('service_type', 'vendor')?.order('created_at', { ascending: false })?.limit(5);
    ...
    it('should have calendar_event_id as non-null string', async () => {
      ...
      const { data: jobsWithCalendarIds, error } = await supabase?.from('jobs')?.select('id, calendar_event_id, service_type')?.eq('service_type', 'vendor')?.not('calendar_event_id', 'is', null)?.limit(5);
  ...
  describe('On-site Job Calendar Fields', () => {
    it('should have NULL scheduled times and correct location', async () => {
      ...
      const { data: onSiteJobs, error } = await supabase?.from('jobs')?.select('id, title, service_type, scheduled_start_time, scheduled_end_time, calendar_event_id, location, color_code')?.eq('service_type', 'in_house')?.order('created_at', { ascending: false })?.limit(5);
    ...
    it('should not have calendar_event_id for on-site jobs', async () => {
      ...
      const { data: onSiteJobsNoCalendar, error } = await supabase?.from('jobs')?.select('id, calendar_event_id, service_type')?.eq('service_type', 'in_house')?.is('calendar_event_id', null)?.limit(10);
  ...
  describe('SQL Probe Verification', () => {
    it('should run SQL probes and print results as per Step 15 spec', async () => {
      ...
      const { data: lastJobs, error } = await supabase?.from('jobs')?.select('id, title, service_type, scheduled_start_time, scheduled_end_time, calendar_event_id, location, color_code')?.order('created_at', { ascending: false })?.limit(5);
      ...
      console.log(`✅ On-site jobs (${onSiteJobs?.length}): All have NULL scheduled times and correct location`);
  ...
  describe('Step 15 PASS Criteria Validation', () => {
    it('should verify PASS criteria: Off-site job calendar linkage', async () => {
      ...
      const { data: offSiteJob, error } = await supabase?.from('jobs')?.select('*')?.eq('service_type', 'vendor')?.single();
    ...
    it('should verify PASS criteria: On-site job calendar linkage', async () => {
      ...
      const { data: onSiteJob, error } = await supabase?.from('jobs')?.select('*')?.eq('service_type', 'in_house')?.single();
    ...
    it('should verify no global promised_date at job creation', async () => {
      ...
      const { data: newJobs, error } = await supabase?.from('jobs')?.select('id, title, service_type, promised_date, created_at')?.order('created_at', { ascending: false })?.limit(1);
