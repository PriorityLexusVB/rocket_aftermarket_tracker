  ...
  const checkDismissed = () => {
    try {
      ...
      if (dismissed) {
        ...
        if (data?.timestamp > fourHoursAgo && data?.jobIds?.length === overdueJobs?.length) {
          return true
        }
      }
      return false
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          <div className="flex-shrink-0">
            <Icon name="AlertTriangle" size={24} className="text-red-500 animate-pulse" />
          </div>
            ...
            <div className="mt-2">
              <div className="flex flex-wrap gap-2">
                {overdueJobs?.slice(0, 3)?.map((job) => (
                  ...
                  >
                    <div
                      className={`w-2 h-2 rounded-full ${getSeverityColor(job?.severity_level)} mr-1`}
                    />
