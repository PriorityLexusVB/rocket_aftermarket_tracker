...
export const useLogger = () => {
  ...
  const logWithUser = useCallback(
    async (level, action, entityType, entityId, description, additionalData = {}) => {
      const enhancedData = {
        ...additionalData,
        userId: user?.id,
  ...
  const logComponentAction = useCallback(
    async (componentName, action, details = {}) => {
      return logWithUser(
        LOG_LEVELS?.INFO,
        action,
        ENTITY_TYPES?.SYSTEM,
  ...
  const logFormSubmission = useCallback(
    async (formName, formData, isSuccess = true) => {
      return logWithUser(
  ...
  const logPageView = useCallback(
    async (pageName, additionalContext = {}) => {
      return logWithUser(
        LOG_LEVELS?.INFO,
  ...
  const logBusinessAction = useCallback(
    async (action, entityType, entityId, description, data = {}) => {
      return logWithUser(LOG_LEVELS?.INFO, action, entityType, entityId, description, data)
    },
    [logWithUser]
  )
  ...
  const logSalesAction = useCallback(
    async (action, saleId, description, data = {}) => {
      return logWithUser(LOG_LEVELS?.INFO, action, ENTITY_TYPES?.SALE, saleId, description, data)
    },
    [logWithUser]
  )

  const logVehicleAction = useCallback(
    async (action, vehicleId, description, data = {}) => {
      return logWithUser(
        LOG_LEVELS?.INFO,
        action,
  ...
  const logJobAction = useCallback(
    async (action, jobId, description, data = {}) => {
      return logWithUser(LOG_LEVELS?.INFO, action, ENTITY_TYPES?.JOB, jobId, description, data)
    },
    [logWithUser]
  )
  ...
  return {
    ...
    // Utility methods
    logInfo: useCallback(
      (action, entityType, entityId, description, data) =>
        logWithUser(LOG_LEVELS?.INFO, action, entityType, entityId, description, data),
      [logWithUser]
    ),
    logWarning: useCallback(
      (action, entityType, entityId, description, data) =>
        logWithUser(LOG_LEVELS?.WARN, action, entityType, entityId, description, data),
      [logWithUser]
    ),
    logSuccess: useCallback(
      (action, entityType, entityId, description, data) =>
        logWithUser(LOG_LEVELS?.SUCCESS, action, entityType, entityId, description, data),
      [logWithUser]
    ),
  }
}
