  ...
  const JobCard = ({ job, style = {}, isDraggable = true }) => {
    ...
    return (
      ...
      >
        ...
        {/* Job content */}
        <div className="text-xs font-medium truncate pr-4">{job?.title}</div>
        ...
        <div className="flex items-center justify-between text-xs mt-1">
          ...
          {job?.vehicle_info && (
            <div className="flex items-center space-x-1">
              ...
              <span className="truncate max-w-20">{job?.vehicle_info}</span>
            </div>
          )}
        </div>
        {/* Overdue indicator */}
                  ...
                  {dayJobs?.slice(0, 3)?.map((job, jobIndex) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div
                          className={`w-2 h-2 rounded-full ml-1 ${getPriorityColor(job?.priority)}`}
                        />
                      </div>
                    </div>
                  ))}
      ...
      <div className="flex-1 overflow-auto">
        <div className="flex">
          ...
          {daysToDisplay?.map((date, dayIndex) => {
            ...
            return (
              <div key={dayIndex} className="flex-1 border-r border-gray-200 relative min-w-0">
                ...
                {dayJobs?.map((job, jobIndex) => (
                  <JobCard key={job?.id || jobIndex} job={job} isDraggable={true} />
                ))}
