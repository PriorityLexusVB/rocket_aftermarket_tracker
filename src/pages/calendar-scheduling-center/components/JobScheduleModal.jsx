...
const JobScheduleModal = ({ job, vendors = [], onClose, onUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      hasConflict &&
      !window.confirm('There is a scheduling conflict. Do you want to proceed anyway?')
    ) {
      return
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-screen overflow-y-auto">
        ...
        {hasConflict && conflictDetails && (
          <div
            className="mx-6 mt-4 bg-red-50 border border-red-200 rounded-lg p-4"
            role="alert"
            aria-live="polite"
          >
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <div className="mt-2 p-2 bg-red-100 rounded text-xs text-red-800">
                  <strong>Conflict with:</strong> {conflictDetails?.customer_name}
                  <br />
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              ...
              />
              {errors?.title && (
                <p id="job-title-error" className="text-red-600 text-sm mt-1" role="alert">
                  {errors?.title}
                </p>
              )}
            </div>
            ...
            <div className="md:col-span-2">
              ...
              {errors?.vendor_id && (
                <p id="vendor-select-error" className="text-red-600 text-sm mt-1" role="alert">
                  {errors?.vendor_id}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              {errors?.scheduled_start_time && (
                <p id="start-time-error" className="text-red-600 text-sm mt-1" role="alert">
                  {errors?.scheduled_start_time}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              {errors?.scheduled_end_time && (
                <p id="end-time-error" className="text-red-600 text-sm mt-1" role="alert">
                  {errors?.scheduled_end_time}
                </p>
              )}
              <p id="duration-info" className="text-xs text-gray-500 mt-1">
                ...
                {formData?.scheduled_start_time && formData?.scheduled_end_time
                  ? Math.round(
                      (new Date(formData.scheduled_end_time) -
                        new Date(formData.scheduled_start_time)) /
                        (1000 * 60)
                    ) + ' minutes'
                  : '—'}
              </p>
