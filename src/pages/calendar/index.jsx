...
const CalendarSchedulingCenter = () => {
  ...
  const loadCalendarData = async () => {
    try {
      ...
      const { data, error } = await supabase?.from('jobs')?.select('*')?.eq('user_id', user?.id)?.order('scheduled_start_time', { ascending: true });
      ...
      return (
        <div className="grid grid-cols-7 gap-1 h-full">
          ...
          {weekDays?.map((day, index) => (
            <div key={`header-${index}`} className="bg-gray-50 p-2 border-b font-semibold text-center">
          ...
          {/* Day content */}
          {weekDays?.map((day, index) => (
            <div key={`content-${index}`} className="bg-white p-2 border-r border-gray-200 min-h-96 overflow-y-auto">
              {day?.jobs?.map((job) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-xs opacity-80">
                      {jobStartTime ? safeFormatTime(jobStartTime, { 
                        hour: 'numeric', 
                        minute: '2-digit' 
                      }) : 'Invalid Time'}
                    </div>
              ...
              <div className="space-y-2">
                <button
                  ...
                  className={`w-full py-2 rounded transition-colors ${
                    viewType === 'day' ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}
                >
                  Day View
                </button>
                <button
                  ...
                  className={`w-full py-2 rounded transition-colors ${
                    viewType === 'week' ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}
                >
                  Week View
                </button>
                <button
                  ...
                  className={`w-full py-2 rounded transition-colors ${
                    viewType === 'month' ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}
                >
                  Month View
                </button>
