  ...
  useEffect(() => {
    const handleKeyPress = (e) => {
      if (selectedJobs?.length > 0) {
        switch (e?.key) {
          case '1': handleBulkStatusUpdate('Pending');
            break;
          case '2': handleBulkStatusUpdate('In Progress');
            break;
          case '3': handleBulkStatusUpdate('Complete');
            break;
        }
      }
    };
          ...
          <tbody>
            {sortedJobs?.map((job) => (
              ...
              >
                ...
                <td className="p-3">
                  <div>
                    <div className="font-medium text-sm">{job?.vehicle?.year} {job?.vehicle?.make} {job?.vehicle?.model}</div>
                    <div className="text-xs text-muted-foreground font-mono">{job?.vehicle?.vin}</div>
                  </div>
                </td>
                <td className="p-3">
                ...
                <td className="p-3">
                  <div className={`text-sm ${isOverdue(job?.dueDate) && job?.status !== 'Complete' ? 'text-error font-medium' : ''}`}>
                    {job?.dueDate}
                  </div>
                </td>
                <td className="p-3">
