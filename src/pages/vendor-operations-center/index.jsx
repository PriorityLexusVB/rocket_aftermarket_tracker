...
const VendorOperationsCenter = () => {
  ...
  const [performanceData] = useState({
    ...
    avgTurnaround: 2.9,
    topPerformers: [
      { id: "VND003", name: "Wrap Experts", specialty: "Vehicle Wraps", completionRate: 96, jobsCompleted: 156 },
      { id: "VND001", name: "AutoTint Pro", specialty: "Window Tinting", completionRate: 94, jobsCompleted: 127 },
      { id: "VND004", name: "Detail Kings", specialty: "Detailing", completionRate: 92, jobsCompleted: 203 },
      { id: "VND005", name: "Glass Guard", specialty: "Windshield Protection", completionRate: 89, jobsCompleted: 78 },
      { id: "VND002", name: "Shield Masters", specialty: "Paint Protection", completionRate: 87, jobsCompleted: 89 }
    ],
    recentActivity: [
      {
        id: "ACT001",
  ...
  const handleBulkAction = (action, vendorIds) => {
    switch (action) {
      case 'sms': console.log('Sending SMS to vendors:', vendorIds);
        // Mock SMS sending
        alert(`SMS sent to ${vendorIds?.length} vendor${vendorIds?.length > 1 ? 's' : ''}`);
        break;
      case 'reassign': console.log('Reassigning jobs for vendors:', vendorIds);
        alert(`Job reassignment initiated for ${vendorIds?.length} vendor${vendorIds?.length > 1 ? 's' : ''}`);
        break;
      case 'export':
        console.log('Exporting vendor data:', vendorIds);
        alert('Vendor data export started. Download will begin shortly.');
        break;
      case 'create': console.log('Creating new vendor');
  ...
  const handleDrillDown = (type, id) => {
    ...
    switch (type) {
      case 'completion-rate':
        ...
        break;
      case 'turnaround-time': alert('Opening turnaround time breakdown');
        break;
      case 'specialty-distribution': alert('Opening specialty distribution details');
        break;
      case 'top-performers':
              ...
              <div className="bg-card p-4 rounded-lg border border-border">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-semibold text-foreground">
                      {Math.round(vendors?.reduce((sum, v) => sum + v?.completionRate, 0) / vendors?.length)}%
                    </p>
                  </div>
                </div>
              </div>
