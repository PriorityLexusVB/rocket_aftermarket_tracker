...
import { Clock, Calendar, Filter, Search, AlertTriangle, CheckCircle, RefreshCw, Users, BarChart3, ArrowUpRight, CheckCheck, UserPlus } from 'lucide-react';
...
const CurrentlyActiveAppointments = () => {
  ...
  useEffect(() => {
    ...
    const subscription = supabase?.channel('job_updates')?.on('postgres_changes', 
  ...
  const loadStaffMembers = async () => {
    try {
      const { data, error } = await supabase?.from('user_profiles')?.select('id, full_name, email, role, department')?.eq('is_active', true)?.order('full_name');
  ...
  const loadUnassignedJobs = async () => {
    try {
      const { data, error } = await supabase?.from('jobs')?.select(`
        ...
        `)?.is('assigned_to', null)?.eq('job_status', 'pending')?.order('created_at', { ascending: false })?.limit(10);
  ...
  const loadPerformanceMetrics = async () => {
    try {
      ...
      const { data: todayJobs, error: todayError } = await supabase?.from('jobs')?.select('job_status, created_at, completed_at')?.gte('created_at', `${today}T00:00:00Z`)?.lte('created_at', `${today}T23:59:59Z`);
      ...
      const { data: weekJobs, error: weekError } = await supabase?.from('jobs')?.select('job_status, created_at, completed_at')?.gte('created_at', `${weekStartStr}T00:00:00Z`);

      if (weekError) throw weekError;

      // Calculate metrics
      const todayCompleted = todayJobs?.filter(job => job?.job_status === 'completed')?.length || 0;
      ...
      const completedJobs = weekJobs?.filter(job => job?.job_status === 'completed' && job?.completed_at && job?.created_at);
  ...
  const loadAppointments = async () => {
    try {
      ...
      const { data, error } = await supabase?.from('jobs')?.select(`
        ...
        `)?.in('job_status', ['pending', 'in_progress', 'scheduled', 'quality_check'])?.order('scheduled_start_time', { ascending: true });
  ...
  const applyFilters = () => {
    ...
    if (searchQuery?.trim()) {
      ...
      filtered = filtered?.filter(apt =>
        ...
        `${apt?.vehicles?.year} ${apt?.vehicles?.make} ${apt?.vehicles?.model}`?.toLowerCase()?.includes(query)
      );
    }
  ...
  const getAppointmentCounts = () => {
    ...
    const inProgress = originalAppointments?.filter(apt => apt?.job_status === 'in_progress')?.length || 0;
    const scheduled = originalAppointments?.filter(apt => apt?.job_status === 'scheduled')?.length || 0;
              ...
              <div className="flex items-center space-x-6">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600 font-medium">Active Appointments & Operations Hub</p>
                  </div>
                </div>
            ...
            {appointments?.length > 0 ? (
              ...
              <div className="text-center py-20">
                ...
                <p className="text-gray-600 mb-8 max-w-md mx-auto">
                  {searchQuery || statusFilter !== 'all' || vendorFilter !== 'all' || priorityFilter !== 'all' 
                    ? 'No appointments match your current filters. Try adjusting your search criteria.' :'All appointments are completed or there are no scheduled appointments at this time.'}
                </p>
              </div>
            )}
          </div>
