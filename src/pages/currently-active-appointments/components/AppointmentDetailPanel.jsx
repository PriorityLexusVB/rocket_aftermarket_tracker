...
const AppointmentDetailPanel = ({ appointment, onClose, onUpdate }) => {
  ...
  const totalEstimatedCost = jobParts?.reduce(
    (sum, part) => sum + (parseFloat(part?.products?.estimated_cost) || 0),
    0
  )
      ...
      <div className="p-6 border-b border-gray-200/50">
        ...
        >
          <div className="flex items-start justify-between mb-4">
            ...
            <div
              className={`inline-flex items-center px-3 py-1.5 rounded-full text-sm font-semibold ${appointment?.statusConfig?.bg} ${appointment?.statusConfig?.text} ${appointment?.statusConfig?.border} border`}
            >
            ...
            <div className="grid grid-cols-3 gap-4 text-sm">
              ...
              {appointment?.actual_cost && (
                <div>
                  ...
                  <div className="text-gray-900 font-semibold">
                    {formatCurrency(appointment?.actual_cost)}
                  </div>
                </div>
              )}
            </div>
