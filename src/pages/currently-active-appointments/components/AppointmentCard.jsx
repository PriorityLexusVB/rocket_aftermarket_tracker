...
import { Calendar, User, Building2, Phone, Mail, AlertTriangle, Car, ArrowRight, Check } from 'lucide-react';

const AppointmentCard = ({ appointment, onClick, onUpdateStatus, isSelected, bulkMode, onToggleSelect }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`absolute top-0 left-0 w-full h-1 ${
        isSelected ? 'bg-gradient-to-r from-indigo-500 to-purple-500' : isOverdue ?'bg-gradient-to-r from-red-500 to-red-600' : 'bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600'
      }`}></div>
        ...
        <div className="absolute top-4 left-4 z-10">
          <div 
            ...
            className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center cursor-pointer transition-all duration-200 ${
              isSelected 
                ? 'bg-indigo-500 border-indigo-500 text-white' :'bg-white border-gray-300 hover:border-indigo-400'
            }`}
          >
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center space-x-4">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center shadow-lg ${
              isSelected ? 'bg-gradient-to-br from-indigo-600 to-indigo-700' : isOverdue ?'bg-gradient-to-br from-red-600 to-red-700' : 'bg-gradient-to-br from-indigo-600 to-purple-700'
            }`}>
          ...
          {/* Status Badge */}
          <div className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold ${appointment?.statusConfig?.bg} ${appointment?.statusConfig?.text} ${appointment?.statusConfig?.border} border`}>
            ...
            <div>
              ...
              <div className="text-sm text-gray-600 space-y-1">
                ...
                <div className="flex items-center space-x-2">
                  <div className={`w-3 h-3 rounded-full`} style={{backgroundColor: appointment?.vehicles?.color?.toLowerCase() || '#gray'}}></div>
            ...
            <div>
              ...
              {timeRemaining && (
                <div className={`text-xs font-semibold ${
                  isOverdue ? 'text-red-600' : timeRemaining?.includes('remaining') ? 'text-green-600' : 'text-orange-600'
                }`}>
                  {timeRemaining}
                </div>
              )}
