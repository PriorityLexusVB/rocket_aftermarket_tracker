...
const SummaryCards = ({ data, stats, onCardClick }) => {
  ...
  const calculateStats = (salesData) => {
    ...
    const completed = salesData?.filter(sale => sale?.status === 'completed' || sale?.status === 'delivered');
    const pending = salesData?.filter(sale => sale?.status === 'pending' || sale?.status === 'in_progress');
