...
const NewSaleModal = ({ isOpen, onClose, onSubmit, staffMembers }) => {
  ...
  const calculateTotals = () => {
    const totalCost = formData?.itemsSold?.reduce(
      (sum, item) => sum + (parseFloat(item?.cost) || 0),
      0
    )
    const totalPrice = formData?.itemsSold?.reduce(
                      ...
                      <div className="h-10 px-3 border border-gray-300 rounded-md flex items-center bg-gray-100">
                        <span
                          className={`text-sm font-medium ${
                            calculateLineItemProfit(item?.cost, item?.price) >= 0
                              ? 'text-green-600'
                              : 'text-red-600'
                          }`}
                        >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                    ...
                    <div className={item?.processingType !== 'vendor' ? 'opacity-50' : ''}>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          handleLineItemChange(index, 'vendorDate', e?.target?.value)
                        }
                      />
                    </div>
