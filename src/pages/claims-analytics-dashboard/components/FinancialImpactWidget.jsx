    ...
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="p-4 bg-blue-50 rounded-lg">
          ...
          <p className="text-xs text-blue-700">vs {formatCurrency(total_estimated_cost)} estimated</p>
        </div>
        ...
        <div className={`p-4 rounded-lg ${parseFloat(net_financial_impact) >= 0 ? 'bg-red-50' : 'bg-green-50'}`}>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className={`text-sm font-medium ${parseFloat(net_financial_impact) >= 0 ? 'text-red-900' : 'text-green-900'}`}>
              Net Impact
            </span>
          </div>
          <p className={`text-lg font-bold ${parseFloat(net_financial_impact) >= 0 ? 'text-red-900' : 'text-green-900'}`}>
            {formatCurrency(net_financial_impact)}
          </p>
          <p className={`text-xs ${parseFloat(net_financial_impact) >= 0 ? 'text-red-700' : 'text-green-700'}`}>
        ...
        <div className="p-4 bg-purple-50 rounded-lg">
          ...
          <p className={`text-lg font-bold ${getPerformanceColor(parseFloat(cost_accuracy_rate), true)}`}>
            {cost_accuracy_rate}%
          </p>
            ...
            <table className="min-w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left py-2 px-4 text-sm font-medium text-gray-900">Category</th>
                  <th className="text-center py-2 px-4 text-sm font-medium text-gray-900">Claims</th>
                  <th className="text-center py-2 px-4 text-sm font-medium text-gray-900">Total Cost</th>
                  <th className="text-center py-2 px-4 text-sm font-medium text-gray-900">Avg Cost</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {category_breakdown?.map((category, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    <td className="py-2 px-4 text-sm font-medium text-gray-900">{category?.category}</td>
                    <td className="py-2 px-4 text-sm text-gray-600 text-center">{category?.claim_count}</td>
