...
const ProductPerformanceMatrix = ({ data, selectedCategory }) => {
  ...
  const getPerformanceScore = (item) => {
    ...
    const revenueScore =
      maxRevenue > 0 ? ((parseFloat(item?.total_revenue) || 0) / maxRevenue) * 50 : 0
    const quantityScore =
      maxQuantity > 0 ? ((item?.total_quantity_sold || 0) / maxQuantity) * 50 : 0
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-400">
            {selectedCategory === 'all'
              ? 'No product categories found with sales data'
