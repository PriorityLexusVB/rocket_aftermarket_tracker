    ...
    if (filterBy !== 'all') {
      switch (filterBy) {
        ...
        case 'medium_performance':
          filtered = filtered?.filter(vendor => vendor?.completion_rate >= 70 && vendor?.completion_rate < 90);
          break;
      ...
      {processedData?.length > 0 ? (
            ...
            <tbody>
              {processedData?.map((vendor, index) => (
                ...
                >
                  ...
                  <td className="py-4 px-4">
                    <div className={`flex items-center space-x-2 px-3 py-1 rounded-full ${getCompletionRateColor(vendor?.completion_rate)}`}>
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-400">
            {filterBy === 'all' ?'No vendor performance data available'
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">
                {(processedData?.reduce((sum, v) => sum + (v?.completion_rate || 0), 0) / processedData?.length)?.toFixed(1)}%
              </div>
              <div className="text-sm text-gray-600">Avg Completion Rate</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-blue-600">
                {processedData?.reduce((sum, v) => sum + (v?.total_products_sold || 0), 0)?.toLocaleString()}
              </div>
              <div className="text-sm text-gray-600">Total Products Sold</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-600">
                ${processedData?.reduce((sum, v) => sum + (parseFloat(v?.total_revenue) || 0), 0)?.toLocaleString()}
              </div>
