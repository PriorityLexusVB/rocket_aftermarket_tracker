...
const EnhancedVehicleTable = ({
  ...
  const getStatusBadge = (status, vehicleId) => {
    ...
    return (
      <div className="flex items-center space-x-2">
        ...
        {hasOverdueJobs && <Icon name="AlertTriangle" size={14} className="text-red-500" />}
      </div>
    )
  }
  ...
  const getRowClassName = (vehicle) => {
    const hasOverdueJobs =
      overdueJobs &&
      overdueJobs?.some(
        (job) => job?.vehicle_info && vehicle && job?.vehicle_info?.includes(vehicle?.license_plate)
      )
            ...
            <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
              <div className="flex items-center space-x-4">
                <span className="text-sm font-medium text-blue-800">
                  Bulk Actions for {selectedVehicles?.length} vehicle
            ...
            <tbody className="bg-card divide-y divide-border">
              {sortedVehicles &&
                sortedVehicles?.map((vehicle) => (
                  <tr key={vehicle?.id} className={getRowClassName(vehicle)}>
