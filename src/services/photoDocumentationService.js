...
export const photoDocumentationService = {
  ...
  async uploadJobPhoto(jobId, vehicleId, file, metadata = {}) {
    try {
      ...
      const { data: uploadData, error: uploadError } = await supabase?.storage?.from('job-photos')?.upload(filePath, file, {
          cacheControl: '3600',
          upsert: false
        });
      ...
      const { data: photoData, error: photoError } = await supabase?.from('job_photos')?.insert({
          job_id: jobId,
  ...
  async getJobPhotos(jobId) {
    try {
      const { data, error } = await supabase?.from('job_photos')?.select(`
          id,
          job_id,
          vehicle_id,
          file_path,
  ...
  async getPhotoSignedUrl(filePath, expiresIn = 3600) {
    try {
      const { data, error } = await supabase?.storage?.from('job-photos')?.createSignedUrl(filePath, expiresIn);
  ...
  async getDocumentationNotes(jobId) {
    try {
      const { data, error } = await supabase?.from('communications')?.select(`
        ...
        `)?.eq('job_id', jobId)?.in('communication_type', ['note', 'photo_documentation'])?.order('sent_at', { ascending: false });
  ...
  async deleteJobPhoto(photoId) {
    try {
      ...
      const { data: photo, error: fetchError } = await supabase?.from('job_photos')?.select('file_path, uploaded_by')?.eq('id', photoId)?.single();
      ...
      // Delete from storage
      const { error: storageError } = await supabase?.storage?.from('job-photos')?.remove([photo?.file_path]);
  ...
  async getFilteredJobPhotos(jobId, filters = {}) {
    try {
