      ...
      if (searchQuery) {
        switch (tableType) {
          case 'jobs':
            ...
            break;
          case 'vehicles':
            query = query?.or(`vin.ilike.%${searchQuery}%,make.ilike.%${searchQuery}%,model.ilike.%${searchQuery}%`);
            break;
          case 'vendors':
...
export const useFilterPresets = (pageType) => {
  ...
  const savePreset = async (name, filters, isPublic = false) => {
    const result = await advancedFeaturesService?.saveFilterPreset(pageType, name, filters, isPublic);
    if (result?.data) {
