...
export const claimsService = {
  ...
  async getCustomerClaims(customerEmail) {
    try {
      const { data, error } = await supabase?.from('claims')?.select(`
          *,
        ...
        `)?.eq('customer_email', customerEmail)?.order('created_at', { ascending: false });
  ...
  async getCustomerVehicles(customerEmail) {
    try {
      const { data, error } = await supabase?.from('vehicles')?.select('id, make, model, year, vin')?.eq('owner_email', customerEmail)?.eq('vehicle_status', 'active');
  ...
  async getProducts() {
    try {
      const { data, error } = await supabase?.from('products')?.select('id, name, brand, category, unit_price')?.order('name');
  ...
  async updateClaim(claimId, updates) {
    try {
      const { data, error } = await supabase?.from('claims')?.update(updates)?.eq('id', claimId)?.select(`
          *,
          vehicle:vehicles(make, model, year),
          product:products(name, brand),
          assigned_to_profile:user_profiles!assigned_to(full_name, email)
  ...
  async uploadClaimPhoto(claimId, file, description) {
    try {
      ...
      // Upload to storage
      const { data: uploadData, error: uploadError } = await supabase?.storage?.from('claim-photos')?.upload(filePath, file);

      if (uploadError) throw uploadError;

      // Save attachment record
      const { data, error } = await supabase?.from('claim_attachments')?.insert([{
          claim_id: claimId,
          file_name: file?.name,
          file_path: filePath,
          file_type: file?.type,
  ...
  async getClaimAttachments(claimId) {
    try {
      ...
      const { data, error } = await supabase?.from('claim_attachments')
        ?.select('*')
        ?.eq('claim_id', claimId)
        ?.order('created_at', { ascending: false });
      ...
      const attachmentPromises = data?.map(async (attachment, index) => {
        try {
          ...
          console.info(`Processing attachment ${index + 1}/${data?.length}: ${attachment?.file_name}`);
      ...
      const validAttachments = processedAttachments?.filter(attachment => 
        attachment && attachment?.id && typeof attachment?.id !== 'string' || !attachment?.id?.startsWith('failed_')
      );

      const successCount = validAttachments?.filter(a => a?.hasValidUrl)?.length;
      const totalCount = validAttachments?.length;
      
      console.info(`Successfully processed ${successCount}/${totalCount} attachments for claim ${claimId}`);
  ...
  async validateClaimAttachments(claimId) {
    try {
      const attachments = await supabase?.from('claim_attachments')
        ?.select('*')
        ?.eq('claim_id', claimId);
  ...
  async getClaimsStats() {
    try {
      const { data, error } = await supabase?.from('claims')?.select('status, priority, claim_amount, created_at');
  ...
  async getStaff() {
    try {
      const { data, error } = await supabase?.from('user_profiles')?.select('id, full_name, email, role')?.in('role', ['admin', 'manager', 'staff'])?.eq('is_active', true)?.order('full_name');
