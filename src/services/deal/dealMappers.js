...
export function mapDbDealToForm(dbDeal) {
  ...
  return {
    ...
    lineItems: (normalized?.job_parts || [])?.map((part, index) => ({
      ...
      no_schedule_reason: part?.no_schedule_reason === 'legacy_unspecified' ? '' : (part?.no_schedule_reason || ''),
      noScheduleReason: part?.no_schedule_reason === 'legacy_unspecified' ? '' : (part?.no_schedule_reason || ''),
