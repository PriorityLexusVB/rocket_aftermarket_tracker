...
class ClaimsAnalyticsService {
  ...
  async getClaimsOverviewMetrics() {
    try {
      ...
      const monthOverMonthChange = previousMonthClaims > 0 
        ? (((currentMonthClaims - previousMonthClaims) / previousMonthClaims) * 100)?.toFixed(1)
        : currentMonthClaims > 0 ? 100 : 0;
      ...
      return {
        ...
        estimated_vs_actual_variance: totalEstimatedCost > 0 
  ...
  async getClaimsByProductType() {
    try {
      const { data, error } = await supabase?.from('job_parts')?.select(`
        ...
        `)?.not('products', 'is', null)?.not('jobs', 'is', null);
      ...
      data?.forEach(item => {
        ...
        if (jobStatus === 'completed') {
          ...
          if (item?.jobs?.completed_at && item?.jobs?.created_at) {
            const resolutionTime = (new Date(item?.jobs?.completed_at) - new Date(item?.jobs?.created_at)) / (1000 * 60 * 60 * 24);
      ...
      return Object.values(categoryGroups)?.map(group => {
        const avgResolution = group?.resolution_times?.length > 0
          ? (group?.resolution_times?.reduce((sum, time) => sum + time, 0) / group?.resolution_times?.length)
          : 0;

        return {
          ...group,
          total_cost: group?.total_cost?.toFixed(2),
          completion_rate: group?.total_claims > 0 
  ...
  async getResolutionTimeTrends(timeframe = '6months') {
    try {
      let dateFilter;
      const now = new Date();
      ...
      const { data, error } = await supabase?.from('jobs')?.select(`
          ...
          actual_cost,
          priority
        `)?.gte('created_at', dateFilter)?.order('created_at', { ascending: true });
      ...
      data?.forEach(job => {
        ...
        if (!monthlyData?.[monthKey]) {
          monthlyData[monthKey] = {
            month: monthKey,
            month_name: createdDate?.toLocaleDateString('en-US', { year: 'numeric', month: 'long' }),
            total_claims: 0,
            completed_claims: 0,
            avg_resolution_time: 0,
            resolution_times: [],
        ...
        if (job?.job_status === 'completed' && job?.completed_at && job?.started_at) {
          const resolutionTime = (new Date(job?.completed_at) - new Date(job?.started_at)) / (1000 * 60 * 60 * 24);
      ...
      return Object.values(monthlyData)?.map(month => ({
        ...month,
        avg_resolution_time: month?.resolution_times?.length > 0
          ? (month?.resolution_times?.reduce((sum, time) => sum + time, 0) / month?.resolution_times?.length)?.toFixed(1)
          : 0,
        completion_rate: month?.total_claims > 0 
  ...
  async getVendorClaimsAnalysis() {
    try {
      ...
      const vendorAnalysis = data?.map(vendor => {
        ...
        const pendingJobs = jobs?.filter(job => job?.job_status === 'pending' || job?.job_status === 'scheduled');
        ...
        let avgResolutionTime = resolvedCount > 0 
        ...
        return {
          ...
          efficiency_score: this.calculateEfficiencyScore(completionRate, avgResolutionTime, costVariance)
        };
  ...
  async getSeasonalClaimPatterns() {
    try {
      const { data, error } = await supabase?.from('jobs')?.select(`
          created_at,
          job_status,
      ...
      const monthly = Object.values(monthlyPatterns)?.sort((a, b) => a?.month - b?.month)
        ?.map(month => ({
          ...month,
          total_cost: month?.total_cost?.toFixed(2),
  ...
  async getFinancialImpactAnalysis() {
    try {
      ...
      data?.forEach(job => {
        ...
        const recovered = job?.transactions?.[0]?.total_amount ? parseFloat(job?.transactions?.[0]?.total_amount) : 0;
      ...
      Object.values(categoryBreakdown)?.forEach(category => {
        category.avg_cost = category?.claim_count > 0 
          ? (category?.total_cost / category?.claim_count)?.toFixed(2)
          : 0;
      ...
      return {
        ...
        category_breakdown: Object.values(categoryBreakdown)?.sort((a, b) => parseFloat(b?.total_cost) - parseFloat(a?.total_cost))
      };
    } catch (error) {
  ...
  async getClaimsDashboardSummary() {
    try {
      const [
        overviewMetrics,
        productTypeClaims,
        ...
        this.getClaimsOverviewMetrics(),
        this.getClaimsByProductType(),
        this.getResolutionTimeTrends('6months'),
        this.getVendorClaimsAnalysis(),
        this.getSeasonalClaimPatterns(),
        this.getFinancialImpactAnalysis()
      ]);
