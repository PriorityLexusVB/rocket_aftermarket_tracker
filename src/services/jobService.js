...
export const jobService = {
  ...
  async getAllJobs(filters = {}) {
    try {
      ...
      console.error('getAllJobs failed:', err?.message || err);
      return [];
    }
  },
  ...
  async getJobById(id) {
    ...
    try {
      ...
      console.error('getJobById failed:', err?.message || err);
      return null;
    }
  },
  ...
  async createJob(dealData = {}) {
    ...
    try {
      ...
      const { data: created, error: jobErr } = await supabase?.from('jobs')?.insert([payload])?.select()?.single();
  ...
  async updateJob(jobId, dealData = {}) {
    ...
    try {
      ...
      const { data: updated, error: jobErr } = await supabase?.from('jobs')?.update(update)?.eq('id', jobId)?.select()?.single();
      if (jobErr) throw jobErr;
      ...
      console.error('updateJob failed:', err?.message || err);
  ...
  async updateStatus(jobId, status, extra = {}) {
    ...
    try {
      ...
      console.error('updateStatus failed:', err?.message || err);
  ...
  async deleteJob(jobId) {
    ...
    try {
      ...
      console.error('deleteJob failed:', err?.message || err);
