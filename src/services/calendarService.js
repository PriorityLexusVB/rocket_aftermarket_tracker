...
export const calendarService = {
  ...
  async createScheduledJob(jobData) {
    try {
      ...
      const { data, error } = await supabase?.from('jobs')?.insert([newJobData])?.select(`
          *,
          vendor:vendors(id, name, specialty),
          vehicle:vehicles(id, make, model, year, owner_name, stock_number)
        `)?.single();
   ...
   */
  async getCalendarStats(dateRange = 'today') {
    try {
      const now = new Date();
      ...
      const { count: totalJobs } = await supabase?.from('jobs')?.select('*', { count: 'exact', head: true })?.neq('job_status', 'cancelled');

      // Get scheduled jobs for the period
      const { count: scheduledJobs } = await supabase?.from('jobs')?.select('*', { count: 'exact', head: true })?.gte('scheduled_start_time', startDate?.toISOString())?.lt('scheduled_start_time', endDate?.toISOString());

      // Get overdue jobs
      const { data: overdueJobs } = await this.getOverdueJobs();

      // Get active vendors
      const { count: activeVendors } = await supabase?.from('vendors')?.select('*', { count: 'exact', head: true })?.eq('is_active', true);
