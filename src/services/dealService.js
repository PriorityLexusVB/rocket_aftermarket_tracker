...
export async function createDeal(formState) {
  ...
  try {
    ...
    if ((normalizedLineItems || []).length > 0) {
      const rows = toJobPartRows(job?.id, normalizedLineItems, { includeTimes: JOB_PARTS_HAS_PER_LINE_TIMES })
      if (rows?.length > 0) {
...
export async function updateDeal(id, formState) {
  ...
  if ((normalizedLineItems || []).length > 0) {
    const rows = toJobPartRows(id, normalizedLineItems, { includeTimes: JOB_PARTS_HAS_PER_LINE_TIMES })
    if (rows?.length > 0) {
      ...
      if (insErr) {
        ...
        if (isMissingColumnError(insErr)) {
          ...
          const { error: retryErr } = await supabase?.from('job_parts')?.insert(retryRows)
          if (retryErr) throw new Error(`Failed to update line items: ${retryErr.message}`)
        } else {
          throw new Error(`Failed to update line items: ${insErr.message}`)
        }
      }
    }
  }
