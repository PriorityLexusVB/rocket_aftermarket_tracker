...
**Last updated:** 2026-06-01 ~17:10 ET · **By:** WORK PC / Claude · **HEAD:** `33c85f2` (Wave D UX fixes shipped + live-verified). Live bundle `index-CxvVJ65b.js`. Prior: `f07ca04` (Wave C calendar contrast) → `fb9c006` (state doc).
...
## Open loops (post-Wave-D)
**Surfaced by 2026-06-01 4-agent UX audit, queued for Rob decision (largest first):**
- [ ] **Alpha-white systematic sweep** — same root cause as Wave C+D ghost-card fixes; 17 more files still use `bg-white/5`, `border-white/10`, `text-gray-200` etc. on the actually-light canvas. Files: `src/pages/calendar-flow-management-center/components/{UnscheduledQueue,VendorLaneView}.jsx`, `src/pages/currently-active-appointments/components/{AppointmentDetailPanel,BulkOperationsPanel,FilterControls,PerformanceWidget}.jsx`, `src/pages/currently-active-appointments/index.jsx`, `src/pages/deals/DealForm.jsx`, `src/pages/vehicles/index.jsx`, `src/components/{common/AdvancedFilters,common/ExportButton,mobile/MobileComponents,ui/MultiSelect,ui/SearchableSelect,ui/ToastProvider}.jsx`, `src/contexts/ThemeContext.jsx`. Estimate: 2-3 hours systematic class swap across the 17 files.
- [ ] **Round-Up discoverability** — Codex + sense-check + browser-tester convergent. Currently only reachable from Calendar toolbar; coordinators on Deals/Dashboard at end-of-day must navigate to Calendar first. Options: (A) add to top nav, (B) add Round-Up button to Dashboard, (C) rename button "Export Daily Report" so its purpose is self-explaining.
- [ ] **"Promise/no-time" language standardization** — Codex finding. Same concept appears as `PROMISE`, `P`, `Needs Time Set`, `No Time Set`, `View Time TBD`, `Promised & Overdue`, "all-day promised items" across 6 surfaces. Decision needed: pick ONE canonical phrase (Codex recommends "Needs Time") and sweep.
- [ ] **Status color drift for Quality Check** — Codex finding. Purple in DealDrawer + lib/time.js; emerald in calendarColors.js. Centralize in one status map.
- [ ] **Calendar nav active state** — Codex finding. `src/components/ui/Navbar.jsx:93-96,189-190` + `src/lib/navigation/calendarNavigation.js:95-98` — active check compares pathname to a string with query params; Calendar can fail to highlight. Surgical fix: normalize URL paths before active check.
- [ ] **Touch targets <44px** — Codex finding. CalendarShell controls + UnscheduledQueue complete buttons + RoundUpModal quick actions use `h-8`/`p-1`. WCAG/Apple HIG recommend 44px minimum for primary operational controls.
- [ ] **"Appointments" nav label vs "Active Appointments" page title** — sense-check + browser-tester convergent. Pick one and align.
- [ ] **Deal → Calendar scheduling discoverability** — browser-tester. From inside a deal, no "Schedule to date" control; user must navigate to Calendar Board → drag. Add inline scheduling field or "Schedule this deal" button in DealDrawer.
- [ ] **Create deal full-page vs Edit deal modal** — browser-tester. Same 2-step form, two different patterns. Pick one.
- [ ] **App identity missing in nav** — ui-polisher. Top-left has only an icon + 4px spacer; no wordmark. Add "Aftermarket Tracker" (or similar).
- [ ] **Notification panel naming consistency** — ui-polisher. Desktop "Notifications & Activity" vs mobile "Recent Activity"; same data, different identity. Consolidate.
- [ ] **`/api/health-deals-rel` returns 500 on every page load** — browser-tester. Backend route issue; doesn't visibly break anything but pollutes console.
- [ ] **Recharts width=-1 zero-dim renders on Analytics** — browser-tester. Chart wrappers not getting explicit height from layout.
